...
#include "PluginJamomaUnity.h"

/** Callback function (used in C/C++) */
void DataReturnValueCallback()
{
    //    TTLogMessage("The data has been updated. \n");
}
...
/** Callback function (used in C/C++) to get data's value back
 ...
 for (int iMessageData = 0 ; iMessageData < numberMessageData ; iMessageData++) {
 if (anObject.instance() == mDataMessageVector[iMessageData].instance())
 {
 // Print the returned value
 TTLogMessage("The message %d has been updated to %s \n", iMessageData, value.toString().data());
 return;
 }
 }
...
{
    TTValue out;
    mApplicationDistant.send("ObjectSend", TTSymbol("/Main:Start"), out);
}
...
{
    TTValue out;
    mApplicationDistant.send("ObjectSend", TTSymbol("/Main:Stop"), out);
}
...
{
    int indexMessage = numberMessageData;
    numberMessageData++;
    
    // Create a message data and set its callback function and baton and some attributes
    TTObject mNewDataMessage = TTObject("Data", "message");
    auto messageVector = mDataMessageVector.end();
    messageVector = mDataMessageVector.insert(messageVector, mNewDataMessage);
    
    // Setup the callback mechanism to get the value back
    TTValue args;
    args = TTValue(mDataMessageVector[indexMessage]);
    mDataMessageVector[indexMessage].set("baton", args);
    mDataMessageVector[indexMessage].set("function", TTPtr(&DataReturnValueCallback));
    
    // Setup the data attributes depending of its use inside the application
    mDataMessageVector[indexMessage].set("type", "string");
    mDataMessageVector[indexMessage].set("description", description);
    
    // Register the message data into the local application at an address
    args = TTValue(messageAddress, mDataMessageVector[indexMessage]);
    ...
    if (err)
    {
        ...
        // Erase this message in the vector of messages and reduce "numberMessageData"
        mDataMessageVector.erase (mDataMessageVector.begin() + indexMessage);
        numberMessageData--;
...
int CreateMessageDataPluginWithCallbackUnity(char messageAddress[], char description[], ValueCallback valueCallback)
{
    ...
    args = TTValue(mDataMessageVector[indexMessage]);
    mDataMessageVector[indexMessage].set("baton", args);
    mDataMessageVector[indexMessage].set("function", TTPtr(valueCallback));
...
void RunScenarioPlugin (char pathName[], float speed)
{
    ...
    // Read the scenario file to fill mScenario
    TTObject    xmlHandler("XmlHandler");
    xmlHandler.set("object", mScenario);
    TTValue     out;
    xmlHandler.send("Read", pathName, out);
    ...
    mEventStatusChangedCallback.set("function", TTPtr(&EventStatusChangedCallback));
    ...
    mScenario.set("speed", TTFloat64(speed));
...
{
    mDataMessageVector[indexMessage].set("description", newValue);
}
...
{
    TTValue v;
    mDataMessageVector[indexMessage].get("description", v);
...
void SetMessagePlugin(int indexMessage, char message[])
{
    ...
    mDataMessageVector[indexMessage].send("Command", message, out);
}

/** Save the namespace of the local application in a XML file
...
int UnregisterMessageDataPlugin(char messageAddress[], int indexMessage)
{
    ...
    {
        // Erase the corresponding element in the vector of messages and reduce "numberMessageData"
        mDataMessageVector.erase (mDataMessageVector.begin() + indexMessage);
        numberMessageData--;
